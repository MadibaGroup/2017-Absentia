...
Figure~\ref{fig:system} illustrates the high level overview of the system and how the participants interact with the DApps. The system is composed of a smart contract, called Absentia-DApp, and a sub-module called PET Sub-DApp. Mix and Match is done offline and the Ethereum blockchain is used to perform the verifications that the trustees performed the steps of Mix and Match correctly. Trustees post the proofs related to randomization and partial decryption on chain.
...
~\textcolor{red}{TO-DO:} TRUFFLE: gas: Gas limit used for deploys. Default is 6721975, block gas limit?
...
Load outputs: load the values for the output column in the table

Create row: creates two PET contracts

Find matching row: find which row is the match after performing 8 PETs

Find matching value: returns the output value in the matching row

Create final decryption: creates PET contract to perform the decryption of the output

withdraw funds and withdraw excess funds: these functions are not in the main flow of the protocol. They are logically different (explained in payments), but they have more or less the same amount of gas costs.

Subtraction: subtraction between user's input and the cell.

T1\&T2 randomization ZKP: proof that they randomized the output of the subtraction correctly. These functions have the same logic, designed as different functions for separating the logic.

T1\&T2 partial dec ZKP: proof that they performed partial decryption correctly. These functions have the same logic, designed as different functions for separating the logic.

Full decryption:

Load final ciphertexts: loads the final output into the contract to decrypt the value.

~\textcolor{red}{TO-DO:}  explain why randomization ZKP T1$>$T2??
