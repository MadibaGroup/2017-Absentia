...
\begin{enumerate}
\item \textbf{Trustee model.} As justified above, we seek an MPC/SFE protocol that lets the input holders (\eg Alice and Bob) offload their inputs to a set of trustees for evaluation. 
...
\item \textbf{Circuit type.} When the function to be evaluated is represented as a circuit, the circuit could be based on logic gates (\ie NAND gates) or arimatic operations (\eg additions and multiplications in a modular group). We are indifferent to this design parameter.
\item \textbf{Elliptic curve operations.} While Ethereum can in theory implement different types of cryptography (RSA groups, integer-based discrete logarithms groups, lattices, \etc), it has native support for its own cryptographic operations (ECDSA siganatures) on the elliptic curve \texttt{secp256k1}. For easy of implementation, we preferred an MPC with the same cryptographic setting. 
\end{enumerate}

One MPC protocol that is sufficient in Mix and Match~\cite{JJ00}. 

We are not aware of an explict \emph{proof} that Mix and Match is publicly verifiable, however every step of the protocol is covered by a trustee issuing a non-interactive zero knowledge proof. In fact, it appears that even when all trustees fully collude, trustees can only break privacy (and not integrity) with the exception of one sub-protocol, as noted in ~\cite{JJ00}, called the \emph{plaintext equality test} (PET). Despite the authors' caution, many have used the PET protocol as if it is publicly verifiable (with some justification based on statistical arguments)~\cite{}. Recently it was shown the statistical arguments are not sufficient but the PET protocol can be made publicly verifiable with a simple additional check on the final output~\cite{}.
