...
contract PET {
    ...
    mapping(string => ciphertext) encryptions; 
    // For DH proof
   ...
   function ecmulVerify(uint256[] memory multiplicand, uint256 scalar, uint256[] memory product) internal  returns(bool verifies)
    ...
    function loadCiphertexts(uint256[] memory alice1, uint256[] memory cell, string[] memory index) public 
    {
        require( step == Steps.depositCiphertexts);
    ...
    function PET_subtract(uint256[] memory multiplication, string[] memory index) public returns(bool)
    {
        require( step == Steps.subtract);
     
        bool res = false;
        ciphertext memory e = encryptions[index[0]];
        uint256[] memory alice1 = e.c1;
        uint256[] memory alice2 = e.c2;

        e = encryptions[index[1]]; 
